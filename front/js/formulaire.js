...
// Function to modify the form
function modifyFormFromJson(card) {
    // We create a floating div to contain the form
    let form = document.createElement('div');
    form.className = 'floating_form';
    ...
    // We add a remove button (removes the card from localStorage and the card list)
    ...
    // We add an event listener to the remove button
    removeButton.addEventListener('click', function() {
        // Load the "cardSelection" from localStorage
        let cardSelection = JSON.parse(localStorage.getItem('cardSelection')) || [];

        // Remove the card from the "cardSelection"
        cardSelection = cardSelection.filter(cardItem => cardItem.name !== card.name);

        // Save the new "cardSelection" to localStorage
        localStorage.setItem('cardSelection', JSON.stringify(cardSelection));
